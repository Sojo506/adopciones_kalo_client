...
const mapEventTypeToForm = (eventType) => ({
  nombre: eventType?.nombre ?? "",
  idEstado: String(eventType?.idEstado ?? ""),
});
...
const EventTypeFormPage = () => {
  ...
  const isEditing = Boolean(idTipoEvento);
  ...
  useEffect(() => {
    document.title = isEditing
      ? "Editar tipo de evento | Dashboard Kalö"
      : "Nuevo tipo de evento | Dashboard Kalö";
  }, [isEditing]);
  ...
  useEffect(() => {
    ...
    const loadEventType = async () => {
      try {
        ...
        const detail = await eventTypesApi.getEventTypeById(idTipoEvento, { force: true });
        reset(mapEventTypeToForm(detail));
      } catch (error) {
        Swal.fire({
          icon: "error",
          title: "No pudimos cargar el tipo de evento",
          text: error?.response?.data?.message || "Volvamos al listado para evitar inconsistencias.",
        });
        navigate("/dashboard/tipos-evento");
      } finally {
    ...
    loadEventType();
  }, [idTipoEvento, isEditing, navigate, reset]);
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      if (isEditing) {
        await eventTypesApi.updateEventType(idTipoEvento, payload);
      } else {
      ...
      Swal.fire({
        icon: "success",
        title: isEditing ? "Tipo de evento actualizado" : "Tipo de evento creado",
        text: isEditing
          ? "Los cambios quedaron guardados correctamente."
          : "El tipo de evento fue creado correctamente.",
      });
    ...
    <div className="dashboard-page">
      <div className="dashboard-page__header mt-4">
        <div>
          ...
          <h1>{isEditing ? "Actualizar tipo de evento" : "Crear tipo de evento"}</h1>
          <p className="dashboard-page__lede">
            Define el nombre del tipo de evento y el estado con el que debe quedar disponible.
          </p>
        </div>
        <Link className="dashboard-btn dashboard-btn--ghost" to="/dashboard/tipos-evento">
          Volver al listado
        </Link>
      </div>
