...
const mapMovementTypeToForm = (movementType) => ({
  nombre: movementType?.nombre ?? "",
  idEstado: String(movementType?.idEstado ?? ""),
});
...
const MovementTypeFormPage = () => {
  const { idTipoMovimiento } = useParams();
  ...
  const isEditing = Boolean(idTipoMovimiento);
  ...
  useEffect(() => {
    document.title = isEditing
      ? "Editar tipo de movimiento | Dashboard Kalö"
      : "Nuevo tipo de movimiento | Dashboard Kalö";
  }, [isEditing]);
  ...
  useEffect(() => {
    ...
    const loadMovementType = async () => {
      try {
        setDetailLoading(true);
        const detail = await movementTypesApi.getMovementTypeById(idTipoMovimiento, {
          force: true,
        });
        reset(mapMovementTypeToForm(detail));
      } catch (error) {
        Swal.fire({
          icon: "error",
          title: "No pudimos cargar el tipo de movimiento",
          text: error?.response?.data?.message || "Volvamos al listado para evitar inconsistencias.",
        });
        navigate("/dashboard/tipos-movimiento");
      } finally {
    ...
    loadMovementType();
  }, [idTipoMovimiento, isEditing, navigate, reset]);
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      if (isEditing) {
        await movementTypesApi.updateMovementType(idTipoMovimiento, payload);
      } else {
        await movementTypesApi.createMovementType(payload);
      }

      Swal.fire({
        icon: "success",
        title: isEditing ? "Tipo de movimiento actualizado" : "Tipo de movimiento creado",
        text: isEditing
          ? "Los cambios quedaron guardados correctamente."
          : "El tipo de movimiento fue creado correctamente.",
      });
    ...
    <div className="dashboard-page">
      <div className="dashboard-page__header mt-4">
        <div>
          ...
          <h1>{isEditing ? "Actualizar tipo de movimiento" : "Crear tipo de movimiento"}</h1>
          <p className="dashboard-page__lede">
            Define el nombre del tipo y el estado con el que quedara disponible para inventario y
            ventas.
          </p>
        </div>
        <Link className="dashboard-btn dashboard-btn--ghost" to="/dashboard/tipos-movimiento">
          Volver al listado
        </Link>
      </div>
...
export default MovementTypeFormPage;
