...
const mapTrackingTypeToForm = (trackingType) => ({
  nombre: trackingType?.nombre ?? "",
  idEstado: String(trackingType?.idEstado ?? ""),
});
...
const TrackingTypeFormPage = () => {
  const { idTipoSeguimiento } = useParams();
  ...
  const isEditing = Boolean(idTipoSeguimiento);
  ...
  useEffect(() => {
    document.title = isEditing
      ? "Editar tipo de seguimiento | Dashboard Kalö"
      : "Nuevo tipo de seguimiento | Dashboard Kalö";
  }, [isEditing]);
  ...
  useEffect(() => {
    ...
    const loadTrackingType = async () => {
      try {
        setDetailLoading(true);
        const detail = await trackingTypesApi.getTrackingTypeById(idTipoSeguimiento, {
          force: true,
        });
        reset(mapTrackingTypeToForm(detail));
      } catch (error) {
        Swal.fire({
          icon: "error",
          title: "No pudimos cargar el tipo de seguimiento",
          text: error?.response?.data?.message || "Volvamos al listado para evitar inconsistencias.",
        });
        navigate("/dashboard/tipos-seguimiento");
      } finally {
    ...
    loadTrackingType();
  }, [idTipoSeguimiento, isEditing, navigate, reset]);
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      if (isEditing) {
        await trackingTypesApi.updateTrackingType(idTipoSeguimiento, payload);
      } else {
        await trackingTypesApi.createTrackingType(payload);
      }

      Swal.fire({
        icon: "success",
        title: isEditing ? "Tipo de seguimiento actualizado" : "Tipo de seguimiento creado",
        text: isEditing
          ? "Los cambios quedaron guardados correctamente."
          : "El tipo de seguimiento fue creado correctamente.",
      });
      ...
      <div className="dashboard-page__header mt-4">
        <div>
          <p className="dashboard-page__eyebrow">
            {isEditing ? "Editar tipo de seguimiento" : "Nuevo tipo de seguimiento"}
          </p>
          <h1>{isEditing ? "Actualizar tipo de seguimiento" : "Crear tipo de seguimiento"}</h1>
          <p className="dashboard-page__lede">
            Define el nombre del tipo de seguimiento y el estado con el que debe quedar disponible.
          </p>
        </div>
        <Link className="dashboard-btn dashboard-btn--ghost" to="/dashboard/tipos-seguimiento">
          Volver al listado
        </Link>
      </div>
...
export default TrackingTypeFormPage;
