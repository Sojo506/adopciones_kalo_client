...
const mapRequestTypeToForm = (requestType) => ({
  nombre: requestType?.nombre ?? "",
  idEstado: String(requestType?.idEstado ?? ""),
});
...
const RequestTypeFormPage = () => {
  const { idTipoSolicitud } = useParams();
  ...
  const isEditing = Boolean(idTipoSolicitud);
  ...
  useEffect(() => {
    document.title = isEditing
      ? "Editar tipo de solicitud | Dashboard Kalö"
      : "Nuevo tipo de solicitud | Dashboard Kalö";
  }, [isEditing]);
  ...
  useEffect(() => {
    ...
    const loadRequestType = async () => {
      try {
        ...
        const detail = await requestTypesApi.getRequestTypeById(idTipoSolicitud, { force: true });
        reset(mapRequestTypeToForm(detail));
      } catch (error) {
        Swal.fire({
          icon: "error",
          title: "No pudimos cargar el tipo de solicitud",
          text: error?.response?.data?.message || "Volvamos al listado para evitar inconsistencias.",
        });
        navigate("/dashboard/tipos-solicitud");
      } finally {
    ...
    loadRequestType();
  }, [idTipoSolicitud, isEditing, navigate, reset]);
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      if (isEditing) {
        await requestTypesApi.updateRequestType(idTipoSolicitud, payload);
      } else {
        await requestTypesApi.createRequestType(payload);
      }

      Swal.fire({
        icon: "success",
        title: isEditing ? "Tipo de solicitud actualizado" : "Tipo de solicitud creado",
        text: isEditing
          ? "Los cambios quedaron guardados correctamente."
          : "El tipo de solicitud fue creado correctamente.",
      });
      ...
      <div className="dashboard-page__header mt-4">
        <div>
          <p className="dashboard-page__eyebrow">
            {isEditing ? "Editar tipo de solicitud" : "Nuevo tipo de solicitud"}
          </p>
          <h1>{isEditing ? "Actualizar tipo de solicitud" : "Crear tipo de solicitud"}</h1>
          <p className="dashboard-page__lede">
            Define el nombre del tipo de solicitud y el estado con el que debe quedar disponible.
          </p>
        </div>
        <Link className="dashboard-btn dashboard-btn--ghost" to="/dashboard/tipos-solicitud">
          Volver al listado
        </Link>
      </div>
...
export default RequestTypeFormPage;
