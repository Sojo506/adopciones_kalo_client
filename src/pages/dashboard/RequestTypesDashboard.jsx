import { useEffect, useMemo, useState } from "react";
import { Link } from "react-router-dom";
import Swal from "sweetalert2";
import * as requestTypesApi from "../../api/requestTypes";
import { useAuth } from "../../hooks/useAuth";

const RequestTypesDashboard = () => {
  const { isAdmin } = useAuth();
  const [requestTypes, setRequestTypes] = useState([]);
  const [loading, setLoading] = useState(true);
  const [deletingId, setDeletingId] = useState(null);
  const [search, setSearch] = useState("");

  const filteredRequestTypes = useMemo(() => {
    const query = search.trim().toLowerCase();

    if (!query) {
      return requestTypes;
    }

    return requestTypes.filter((requestType) =>
      [requestType.idTipoSolicitud, requestType.nombre, requestType.estado]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
        .includes(query),
    );
  }, [requestTypes, search]);

  const loadRequestTypes = async () => {
    try {
      setLoading(true);
      const data = await requestTypesApi.getRequestTypes({ force: true });
      setRequestTypes(Array.isArray(data) ? data : []);
    } catch (error) {
      Swal.fire({
        icon: "error",
        title: "No pudimos cargar los tipos de solicitud",
        text: error?.response?.data?.message || "Intenta nuevamente en un momento.",
      });
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    document.title = "Tipos de solicitud | Dashboard Kalö";
  }, []);

  useEffect(() => {
    if (!isAdmin) {
      setLoading(false);
      return;
    }

    loadRequestTypes();
  }, [isAdmin]);

  const onDelete = async (requestType) => {
    const result = await Swal.fire({
      icon: "warning",
      title: "Eliminar tipo de solicitud",
      text: `Se desactivara el tipo "${requestType.nombre}".`,
      showCancelButton: true,
      confirmButtonText: "Eliminar",
      cancelButtonText: "Cancelar",
    });

    if (!result.isConfirmed) {
      return;
    }

    try {
      setDeletingId(requestType.idTipoSolicitud);
      await requestTypesApi.deleteRequestType(requestType.idTipoSolicitud);
      await loadRequestTypes();

      Swal.fire({
        icon: "success",
        title: "Tipo de solicitud eliminado",
        text: "El tipo de solicitud fue desactivado correctamente.",
      });
    } catch (error) {
      Swal.fire({
        icon: "error",
        title: "No pudimos eliminar",
        text: error?.response?.data?.message || "Intenta nuevamente en un momento.",
      });
    } finally {
      setDeletingId(null);
    }
  };

  if (!isAdmin) {
    return (
      <div className="dashboard-page">
        <section className="dashboard-card">
          <p className="dashboard-page__eyebrow">Acceso restringido</p>
          <h1>Tipos de solicitud</h1>
          <p className="dashboard-page__lede">
            Solo un administrador puede gestionar esta tabla desde el dashboard.
          </p>
        </section>
      </div>
    );
  }

  return (
    <div className="dashboard-page">
      <div className="dashboard-page__header mt-4">
        <div>
          <p className="dashboard-page__eyebrow">Gestion de catalogo</p>
          <h1>Tipos de solicitud</h1>
          <p className="dashboard-page__lede">
            Administra los tipos de formulario o solicitud que luego utilizan los flujos de
            adopcion del sistema.
          </p>
        </div>
        <Link className="dashboard-btn dashboard-btn--primary" to="/dashboard/tipos-solicitud/nuevo">
          Crear tipo
        </Link>
      </div>

      <section className="dashboard-card">
        <div className="dashboard-alert">
          No puedes eliminar un tipo de solicitud si todavia tiene solicitudes activas asociadas.
        </div>

        <div className="dashboard-toolbar dashboard-toolbar--between">
          <input
            className="form-control dashboard-search"
            disabled={loading || deletingId !== null}
            onChange={(event) => setSearch(event.target.value)}
            placeholder="Buscar por ID, nombre o estado"
            value={search}
          />
          <span className="dashboard-muted">
            {filteredRequestTypes.length} de {requestTypes.length} tipos
          </span>
        </div>

        {loading ? (
          <div className="dashboard-empty-state">Cargando tipos de solicitud...</div>
        ) : filteredRequestTypes.length === 0 ? (
          <div className="dashboard-empty-state">
            No hay tipos de solicitud que coincidan con tu busqueda.
          </div>
        ) : (
          <div className="dashboard-table-wrap">
            <table className="dashboard-table">
              <thead>
                <tr>
                  <th>ID</th>
                  <th>Nombre</th>
                  <th>Estado</th>
                  <th>Acciones</th>
                </tr>
              </thead>
              <tbody>
                {filteredRequestTypes.map((requestType) => {
                  const isDeletingCurrent = deletingId === requestType.idTipoSolicitud;

                  return (
                    <tr key={requestType.idTipoSolicitud}>
                      <td>{requestType.idTipoSolicitud}</td>
                      <td>{requestType.nombre}</td>
                      <td>{requestType.estado || requestType.idEstado}</td>
                      <td>
                        <div className="dashboard-table__actions">
                          <Link
                            className={`dashboard-btn dashboard-btn--ghost${deletingId !== null ? " is-disabled" : ""}`}
                            onClick={(event) => {
                              if (deletingId !== null) {
                                event.preventDefault();
                              }
                            }}
                            to={`/dashboard/tipos-solicitud/${requestType.idTipoSolicitud}/editar`}
                          >
                            Editar
                          </Link>
                          <button
                            className="dashboard-btn dashboard-btn--danger"
                            disabled={deletingId !== null}
                            onClick={() => onDelete(requestType)}
                            type="button"
                          >
                            {isDeletingCurrent ? "Eliminando..." : "Eliminar"}
                          </button>
                        </div>
                      </td>
                    </tr>
                  );
                })}
              </tbody>
            </table>
          </div>
        )}
      </section>
    </div>
  );
};

export default RequestTypesDashboard;
