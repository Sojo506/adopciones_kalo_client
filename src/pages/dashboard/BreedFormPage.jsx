...
const mapBreedToForm = (breed) => ({
  nombre: breed?.nombre ?? "",
  idEstado: String(breed?.idEstado ?? ""),
});
...
const BreedFormPage = () => {
  ...
  const isEditing = Boolean(idRaza);
  ...
  useEffect(() => {
    document.title = isEditing ? "Editar raza | Dashboard Kalö" : "Nueva raza | Dashboard Kalö";
  }, [isEditing]);
  ...
  useEffect(() => {
    ...
    const loadBreed = async () => {
      try {
        ...
        const detail = await breedsApi.getBreedById(idRaza, { force: true });
        reset(mapBreedToForm(detail));
      } catch (error) {
        Swal.fire({
          icon: "error",
          title: "No pudimos cargar la raza",
          text: error?.response?.data?.message || "Volvamos al listado para evitar inconsistencias.",
        });
        navigate("/dashboard/razas");
      } finally {
    ...
    loadBreed();
  }, [idRaza, isEditing, navigate, reset]);
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      if (isEditing) {
        await breedsApi.updateBreed(idRaza, payload);
      } else {
        await breedsApi.createBreed(payload);
      }

      Swal.fire({
        icon: "success",
        title: isEditing ? "Raza actualizada" : "Raza creada",
        text: isEditing
          ? "Los cambios quedaron guardados correctamente."
          : "La raza fue creada correctamente.",
      });
  ...
  return (
    <div className="dashboard-page">
      <div className="dashboard-page__header mt-4">
        <div>
          <p className="dashboard-page__eyebrow">{isEditing ? "Editar raza" : "Nueva raza"}</p>
          <h1>{isEditing ? "Actualizar raza" : "Crear raza"}</h1>
          <p className="dashboard-page__lede">
            Define el nombre de la raza y el estado con el que debe quedar disponible.
          </p>
        </div>
        <Link className="dashboard-btn dashboard-btn--ghost" to="/dashboard/razas">
          Volver al listado
        </Link>
      </div>
          ...
          <form className="dashboard-form" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="dashboard-form__actions">
              <button className="dashboard-btn dashboard-btn--primary" disabled={saving} type="submit">
                {saving ? "Guardando..." : isEditing ? "Guardar cambios" : "Crear raza"}
              </button>
              <Link className="dashboard-btn dashboard-btn--ghost" to="/dashboard/razas">
                Cancelar
              </Link>
