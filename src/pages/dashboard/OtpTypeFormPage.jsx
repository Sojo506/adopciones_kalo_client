...
const mapOtpTypeToForm = (otpType) => ({
  nombre: otpType?.nombre ?? "",
  idEstado: String(otpType?.idEstado ?? ""),
});
...
const OtpTypeFormPage = () => {
  const { idTipoOtp } = useParams();
  ...
  const isEditing = Boolean(idTipoOtp);
  ...
  useEffect(() => {
    document.title = isEditing ? "Editar tipo OTP | Dashboard Kalö" : "Nuevo tipo OTP | Dashboard Kalö";
  }, [isEditing]);
  ...
  useEffect(() => {
    ...
    const loadOtpType = async () => {
      try {
        setDetailLoading(true);
        const detail = await otpTypesApi.getOtpTypeById(idTipoOtp, { force: true });
        reset(mapOtpTypeToForm(detail));
      } catch (error) {
        Swal.fire({
          icon: "error",
          title: "No pudimos cargar el tipo OTP",
          text: error?.response?.data?.message || "Volvamos al listado para evitar inconsistencias.",
        });
        navigate("/dashboard/tipos-otp");
      } finally {
    ...
    loadOtpType();
  }, [idTipoOtp, isEditing, navigate, reset]);
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      if (isEditing) {
        await otpTypesApi.updateOtpType(idTipoOtp, payload);
      } else {
        await otpTypesApi.createOtpType(payload);
      }

      Swal.fire({
        icon: "success",
        title: isEditing ? "Tipo OTP actualizado" : "Tipo OTP creado",
        text: isEditing
          ? "Los cambios quedaron guardados correctamente."
          : "El tipo OTP fue creado correctamente.",
      });
  ...
  return (
    <div className="dashboard-page">
      <div className="dashboard-page__header mt-4">
        <div>
          <p className="dashboard-page__eyebrow">{isEditing ? "Editar tipo OTP" : "Nuevo tipo OTP"}</p>
          <h1>{isEditing ? "Actualizar tipo OTP" : "Crear tipo OTP"}</h1>
          <p className="dashboard-page__lede">
            Define el nombre del tipo OTP y el estado con el que debe quedar disponible.
          </p>
        </div>
        <Link className="dashboard-btn dashboard-btn--ghost" to="/dashboard/tipos-otp">
          Volver al listado
        </Link>
      </div>
          ...
          <form className="dashboard-form" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="dashboard-form__actions">
              <button className="dashboard-btn dashboard-btn--primary" disabled={saving} type="submit">
                {saving ? "Guardando..." : isEditing ? "Guardar cambios" : "Crear tipo OTP"}
              </button>
              <Link className="dashboard-btn dashboard-btn--ghost" to="/dashboard/tipos-otp">
                Cancelar
              </Link>
...
export default OtpTypeFormPage;
