...
const mapSexToForm = (sex) => ({
  nombre: sex?.nombre ?? "",
  idEstado: String(sex?.idEstado ?? ""),
});
...
const SexFormPage = () => {
  ...
  const isEditing = Boolean(idSexo);
  ...
  useEffect(() => {
    document.title = isEditing ? "Editar sexo | Dashboard Kalö" : "Nuevo sexo | Dashboard Kalö";
  }, [isEditing]);
  ...
  useEffect(() => {
    ...
    const loadSex = async () => {
      try {
        ...
        const detail = await sexesApi.getSexById(idSexo, { force: true });
        reset(mapSexToForm(detail));
      } catch (error) {
        Swal.fire({
          icon: "error",
          title: "No pudimos cargar el sexo",
          text: error?.response?.data?.message || "Volvamos al listado para evitar inconsistencias.",
        });
        navigate("/dashboard/sexos");
      } finally {
    ...
    loadSex();
  }, [idSexo, isEditing, navigate, reset]);
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      if (isEditing) {
        await sexesApi.updateSex(idSexo, payload);
      } else {
        await sexesApi.createSex(payload);
      }

      Swal.fire({
        icon: "success",
        title: isEditing ? "Sexo actualizado" : "Sexo creado",
        text: isEditing
          ? "Los cambios quedaron guardados correctamente."
          : "El sexo fue creado correctamente.",
      });
  ...
  return (
    <div className="dashboard-page">
      <div className="dashboard-page__header mt-4">
        <div>
          <p className="dashboard-page__eyebrow">{isEditing ? "Editar sexo" : "Nuevo sexo"}</p>
          <h1>{isEditing ? "Actualizar sexo" : "Crear sexo"}</h1>
          <p className="dashboard-page__lede">
            Define el nombre del sexo y el estado con el que debe quedar disponible.
          </p>
        </div>
        <Link className="dashboard-btn dashboard-btn--ghost" to="/dashboard/sexos">
          Volver al listado
        </Link>
      </div>
          ...
          <form className="dashboard-form" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="dashboard-form__actions">
              <button className="dashboard-btn dashboard-btn--primary" disabled={saving} type="submit">
                {saving ? "Guardando..." : isEditing ? "Guardar cambios" : "Crear sexo"}
              </button>
              <Link className="dashboard-btn dashboard-btn--ghost" to="/dashboard/sexos">
                Cancelar
              </Link>
