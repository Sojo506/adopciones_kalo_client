...
const mapCategoryToForm = (category) => ({
  ...
  idEstado: String(category?.idEstado ?? ""),
});
...
const CategoryFormPage = () => {
  ...
  useEffect(() => {
    document.title = isEditing ? "Editar categoria | Dashboard Kalö" : "Nueva categoria | Dashboard Kalö";
  }, [isEditing]);
  ...
  useEffect(() => {
    ...
    const loadCategory = async () => {
      try {
        ...
        const detail = await categoriesApi.getCategoryById(idCategoria, { force: true });
        reset(mapCategoryToForm(detail));
      } catch (error) {
        Swal.fire({
          icon: "error",
    ...
    loadCategory();
  }, [idCategoria, isEditing, navigate, reset]);
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      if (isEditing) {
        await categoriesApi.updateCategory(idCategoria, payload);
      } else {
  ...
  return (
    <div className="dashboard-page">
      <div className="dashboard-page__header mt-4">
        <div>
          <p className="dashboard-page__eyebrow">{isEditing ? "Editar categoria" : "Nueva categoria"}</p>
          <h1>{isEditing ? "Actualizar categoria" : "Crear categoria"}</h1>
          <p className="dashboard-page__lede">
            Define el nombre comercial de la categoria y el estado con el que debe quedar disponible.
          </p>
        </div>
        <Link className="dashboard-btn dashboard-btn--ghost" to="/dashboard/categorias">
          Volver al listado
        </Link>
      </div>
